...
- Storage: SQLite locally; managed cloud database recommended for Vercel production
- Security: JWT, bcrypt, AES-256-CBC, SHA-256
- Optional intelligence adapters: Transformers, ChromaDB, Redis
- Deployment: One Vercel project with same-origin `/api` routing through Mangum

Presenter note: The MVP runs locally with SQLite and can deploy as one Vercel project for demos. Since Vercel storage is ephemeral, production workloads should use Turso/libSQL or PostgreSQL.
...
- Deploys through a simple public Vercel workflow with environment-based secrets.
