...
- For a hosted demo, open the deployed Vercel URL; the app uses same-origin `/api` calls.
...
Return to the home page, show the architecture diagram, or briefly show the deployed Vercel URL.
...
## Optional hosted-demo note

If recording from Vercel, explain that the frontend and FastAPI function are
deployed from the same repository. The demo uses environment variables for
JWT and encryption secrets. Local SQLite is suitable for development and
demonstration; durable production data should use a managed database because
